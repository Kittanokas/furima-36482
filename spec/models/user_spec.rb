  ...
  describe '新規登録' do
    ...
    context '新規登録できない場合' do
      ...
      it '姓（全角）に半角文字が含まれていると登録できない' do
        ...
        @user.valid?
        expect(@user.errors.full_messages).to include('姓(全角)で入力してください ')
      end
      it '名（全角）に半角文字が含まれていると登録できない' do
        @user.Kanji_first_name = 'taro'
        @user.valid?
        expect(@user.errors.full_messages).to include('名(全角)で入力してください ')       
      end
      it '姓（カナ）にカタカナ以外の文字（平仮名・漢字・英数字・記号）が含まれていると登録できない' do
        @user.Kana_last_name = '鈴木'
        @user.valid?
        expect(@user.errors.full_messages).to include('姓(カナ)で入力してください ')
      end
      it '名（カナ）にカタカナ以外の文字（平仮名・漢字・英数字・記号）が含まれていると登録できない' do
        @user.Kanji_first_name = 'たろう'
        @user.valid?
        expect(@user.errors.full_messages).to include('名(カナ)で入力してください ')
      end
      end
    end
  end
